...
// 邮箱、且通知策略的 severities 包含该等级时才分发；Voice/SMS 列由 dispatcher
...
export function NotificationMatrixCard() {
  const cell = (v: Cell) => (
    <span style={{ color: v === '✓' ? '#52c41a' : '#444', fontWeight: 700 }}>{v}</span>
  )
  return (
    <Alert
      type="info"
      ...
      description={
        <div style={{ marginTop: 8 }}>
          <table style={{ borderCollapse: 'collapse', fontSize: 12, marginBottom: 8 }}>
            ...
            <tbody>
              {MATRIX.map((r) => (
                <tr key={r.sev} style={{ borderTop: '1px solid #1e1e1e' }}>
          ...
          <div style={{ color: '#888', fontSize: 12, lineHeight: '20px' }}>
